...
class Captions_Generator:
    ...
    def __init__(self, args):
        ...
        self.args = args
        self.sampling_params = SamplingParams(temperature=0.7, top_p=0.95, max_tokens=2048, presence_penalty = 1.0, frequency_penalty=1.,
        )

        seed = random.randint(0, 1000000)
        
        self.llm = LLM(
            model="./LLMs/Mistral-7B-Instruct-v0.2", dtype="float16", seed=seed, tensor_parallel_size=1
        )
    ...
    def generate_captions(self, concepts):
        ...
        with open(os.path.join(save_path, f"{self.args.generation_idx}.json"), "w") as f:
